...
import { Scale, Loader2, LogIn, Mail, Lock, CheckCircle2 } from "lucide-react";
import { LogoLink } from "@/components/LogoLink";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";

export default function BalanceHorasLogin() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (manager && manager.role === "admin") {
        navigate("/balance-horas", { replace: true });
        return;
      }
      setIsLoading(false);
    }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success("Bienvenido al módulo Balance de Horas");
        navigate("/balance-horas", { replace: true });
        ...
        <CardHeader className="space-y-2 text-center px-4 sm:px-6">
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <CardTitle className="text-2xl md:text-3xl font-semibold text-primary tracking-tight">
              {showSetPassword ? "Configurar contraseña" : "Balance de Horas"}
            </CardTitle>
          </div>
          ...
          {showSetPassword ? (
            ...
            <>
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <Button type="submit" className="w-full rounded-xl font-normal h-12 text-base transition-all duration-200 active:scale-[0.98]" disabled={isSubmitting}>
                  {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Accediendo...</> : <><LogIn className="mr-2 h-4 w-4" />Iniciar sesión</>}
                </Button>
              </form>
