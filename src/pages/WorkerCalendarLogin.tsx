...
const SALIX_CALENDAR_URL = (workerNumber: string) =>
  `https://salix.verdnatura.es/#/worker/${workerNumber}/calendar`;

const WorkerCalendarLoginContent = () => {
  const navigate = useNavigate();
  const { t, isRTL } = useLanguage();

  const handleAuthenticated = useCallback(async (workerNumber: string) => {
    if (workerNumber) window.open(SALIX_CALENDAR_URL(workerNumber), '_blank');
    navigate("/mi-horario", { replace: true });
  }, [navigate]);

  const handleSessionAlreadyValid = useCallback(async (session: Session) => {
    if (!session.user?.user_metadata?.worker_id) return;
    const wn = session.user.user_metadata?.worker_number as string | undefined;
    if (wn) window.open(SALIX_CALENDAR_URL(wn), '_blank');
    navigate('/mi-horario', { replace: true });
  }, [navigate]);
      ...
      <main className="flex-1 flex items-center justify-center px-3 py-4 sm:p-4">
        <Card className="w-full max-w-md shadow-xl border-border/50 overflow-hidden">
          <CardHeader className="text-center pb-4 px-4 sm:px-6">
            ...
            <CardTitle className="text-xl sm:text-2xl">{t("myCalendar")}</CardTitle>
            <CardDescription className="text-sm">
              {t("calendarLoginSubtitle")}
            </CardDescription>
          </CardHeader>
...
const WorkerCalendarLogin = () => <WorkerCalendarLoginContent />;

export default WorkerCalendarLogin;
