...
const ManagerLogin = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && manager) {
      ...
      // Restricted "candidaturas only" managers
      if ((manager as any).candidaturas_only) {
        navigate("/candidaturas", { replace: true });
        setCheckingSession(false);
        return;
      }

      let targetPath = "/manager";
      if (manager.role === "admin") {
        targetPath = "/admin";
      } else if (manager.role === "consulta") {
        targetPath = "/consulta";
      }
      if (requestId && manager.role === "manager") {
        navigate(`${targetPath}?request=${requestId}`);
      } else {
        navigate(targetPath);
      }
    }
    setCheckingSession(false);
  }, [isAuthenticated, manager, navigate, requestId, redirectPath]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      // Role-based redirect handled by the useEffect above
      ...
      <Card className="w-full max-w-md shadow-xl border-border/50 opacity-0 animate-scale-in overflow-hidden">
        <CardHeader className="space-y-2 text-center px-4 sm:px-6">
          ...
          <CardTitle className="text-2xl md:text-3xl font-semibold text-primary tracking-tight">
            {showSetPassword ? "Configurar contraseña" : "Panel de Encargado"}
          </CardTitle>
